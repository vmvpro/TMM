...
Partial Public Class cls_Povsun_2
    ...
    Sub test(counter As Long, v As Integer)
        ...
        Dim dr_Mehanizm = StoredProcedure.RowZD(Table.Z2_Mehanizm, counter, k)
        MechanismDraw(dr_Mehanizm, 1)

        Dim dr_Speed = StoredProcedure.RowZD(Table.Z2_Speed, counter, k)
        Mechanism_SpeedDraw(dr_Speed)

        Dim dr_Acceleration = StoredProcedure.RowZD(Table.Z2_Acceleration, counter, k)
        ...
        '    If k = i Then
        '        Dim dr_Mehanizm = StoredProcedure.RowZD(Table.Z2_Mehanizm, counter, i)
        '        MechanismDraw(dr_Mehanizm, 1)
        '    Else
        '        Dim dr_Mehanizm = StoredProcedure.RowZD(Table.Z2_Mehanizm, counter, i)
        ...
        '    Dim dr_Speed = StoredProcedure.RowZD(Table.Z2_Speed, counter, i)
        '    Mechanism_SpeedDraw(dr_Speed)
    ...
    Public Sub DrawOne(numberPosition As Integer)
        ...
        Dim dr_Mehanizm = StoredProcedure.RowZD(Table.Z2_Mehanizm, counter, numberPosition)
        MechanismDraw(dr_Mehanizm, 1)

        dt_Speed = db.ReturnDataTable(TableName.Z2_speed)
        Dim dr_Speed = StoredProcedure.RowZD(dt_Speed, counter, numberPosition)
        Mechanism_SpeedDraw(dr_Speed)

        dt_Acceleration = db.ReturnDataTable(TableName.Z2_acceleration)
    ...
    Public Sub MechanismConstruction(tran As SQLiteTransaction, numberPosition As Integer)
        ...
        '==================================================
        'DataBase.LoadDataAdapterAndDataTable(TableName.Z2_mehanizm, da_Mehanizm, dt_Mehanizm)

        ' Dim dr_Mehanizm As DataRow = DataBase.Row(dt_Mehanizm, "id_Z2", counter)
        ...
        ' вертикальная линия ниже точки А
        Dim line_CordinateLineC As New PointDouble(0, -1 * (L_AB + L_BC))
        ...
        'mp_C0 = doc2D.myCalculateCordinatePoint(mp_A, L_AB + L_BC, line_CordinateLineC, enLocation.Vertical_Down)
        mp_C0 = New PointDouble(0, -1 * (L_AB + L_BC))
        ...
        mp_C = doc2D.myCalculateCordinatePoint(mp_B, L_BC, line_CordinateLineC, 270, myDoc2D.enLocation.Vertical_Down)
        ...
        Dim angle_BD As Double = doc2D.myCalculateAngleTwoPoint(mp_C, mp_B)
        'dr_Mehanizm("angle_BD") = angle_BD

        mp_D = doc2D.myCalculateCordinatePoint(mp_B, L_BD, angle_BD)
        ...
        mp_S3 = doc2D.myCalculateCordinateCentreSeg(mp_C, mp_D)
    ...
    Public Sub Mechanism_Speed(tran As SQLiteTransaction, numberPosition As Integer)
        ...
        'DataBase.LoadDataAdapterAndDataTable(TableName.Z2_Speed, da_Speed, dt_Speed)

        'Dim dr_Speed As DataRow = DataBase.Row(dt_Speed, "id_Z2", counter)
        ...
        Dim angle_py As Double = doc2D.myCalculateAngleTwoPoint(New PointDouble(0, enAngle.Angle270), vp_P)
        dr_Speed("angle_py") = angle_py

        'Dim angle_px As Double = doc2D.myCalculateAngleTwoPoint(vp_P, New PointDouble(100, 0))
        'dr_Speed("angle_px") = angle_px
        ...
        vl_bd = v_bd / mashtab_mv
        dr_Speed("v_bd") = v_bd
        dr_Speed("vl_bd") = vl_bd
        'cd_v(numberPosition) = v_bd
    ...
    Public Sub Mechanism_Acceleration(tran As SQLiteTransaction, numberPosition As Integer)
        ...
        'DataBase.LoadDataAdapterAndDataTable(TableName.Z2_acceleration, da_Acceleration, dt_Acceleration)

        'Dim dr_Acceleration As DataRow = DataBase.Row(dt_Acceleration, "id_Z2", counter)
        ...
        dr_Acceleration("a_bd") = a_bd
        dr_Acceleration("al_bd") = al_bd

        Dim angle_bd As Double = doc2D.myCalculateAngleTwoPoint(ap_c, ap_b)
        dr_Acceleration("angle_bd") = angle_bd
        ...
        ap_s3 = doc2D.myCalculateCordinateCentreSeg(ap_c, ap_d)
